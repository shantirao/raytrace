function [z, dxdy] = zernikeIndex(c, xy, R)
...
% [z, dxdy] = zernikeIndex([0.1,.4,0.8],[x(:),y(:)],R)

% change order to match James Wyant https://wp.optics.arizona.edu/jcwyant/wp-content/uploads/sites/13/2016/08/ZernikePolynomialsForTheWeb.pdf
x = xy(:,1)/R;
y = xy(:,2)/R;
z = 0;
dx = 0;
...
N = numel(c);
if N > 55
   error('I can only handle zernike modes up to 9th order (55 of them)');
end
